...
#backWall.placeAbove( deck, offset = 0 )
backWall.placeSouthOf( deck, offset = 0 )

frontWall = BoxThing( label = 'front wall', length = wallThickness, width = 8*12, height = 48, aColor = color.red )
#frontWall.placeAbove( deck, offset = 0 )
...
for y in ( north -20, north - 40, north - 60, north - 80 ):
    ...
    eastPorts.add( PipeThing( label = 'east porthole ' + str(y),
                             endA = vector( east, y, up ),
                             endB = vector( east + portholeThickness, y, up ),
                             diameter = portholeDiameter,
                             aColor = color.green,
                             aOpacity = 0.25 )
                   )
    westPorts.add( PipeThing( label = 'west porthole ' + str(y),
                             endA = vector( west, y, up ),
                             endB = vector( west + portholeThickness, y, up ),
                             diameter = portholeDiameter,
                             aColor = color.green,
                             aOpacity = 0.25 )
                   )
...
westPorts.add( PipeThing( label = 'head porthole',
                              endA = vector( west, toilet_north, up ),
                              endB = vector( west + portholeThickness, toilet_north, up ),
                              diameter = portholeDiameter,
                              aColor = color.white,
                              aOpacity = 0.5 )
               )
...
beamCenter.placeEastOf( westWall, offset = ( deck.width / 2) )
